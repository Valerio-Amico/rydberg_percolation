...
gamma_def = 4.4 # dephasing rate [MHz]
...
for i, p_facilitation in enumerate(p_facilitation_range):
    ...
    for _ in range(just_repetitions):
        clu = ryperc.cluster3D(N_points, shape, distribution="uniform")#, MOT_radius=radius_MOT)
...
for i in range(len(p_facilitation_range)):
    plt.plot(just_repetitions * [p_facilitation_range[i]], N_excts[i], linestyle="", marker="o")
plt.show()
